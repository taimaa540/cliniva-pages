...
const TimePicker: React.FC<TimePickerProps> = ({ defaultValue, onTimeChange }) => {
  const { t } = useTranslation();
  const [selectedTime, setSelectedTime] = useState<string>('');
  const [isPickerOpen, setIsPickerOpen] = useState(false);
  const pickerRef = useRef<HTMLDivElement>(null);

  // معالجة القيمة الافتراضية عند التحميل
  useEffect(() => {
    if (defaultValue) {
  ...
  // إغلاق منتقي الوقت عند النقر خارجها
  useEffect(() => {
  ...
  // الخيارات المحددة مسبقًا للمدة
  const durationPresets = [15, 30, 45, 60, 90, 120];

  return (
    <div className="flex gap-5  items-center">
      <label className="w-44 [font-family:'Lato',Helvetica] text-text-primary font-semibold text-base">
      ...
      <div className="relative w-[360px]">
        ...
        <input
          ...
          readOnly
          value={selectedTime}
          className="h-12 pr-12 bg-background-secondary border-border-light border rounded-md w-full px-4 text-text-primary focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="Select time"
        />

        {/* الأيقونة على اليمين */}
        <div 
          className="absolute right-3 top-1/2 text-text-primary -translate-y-1/2 w-6 h-6 text-gray-400 cursor-pointer"
        ...
        {isPickerOpen && (
          ...
          >
            <h3 className="font-semibold mb-3 text-text-primary">Select Time</h3>
            
            {/* اختيار الوقت بالتنسيق 24 ساعة */}
            <div className="mb-4">
              <h4 className="text-sm font-medium mb-2 text-text-primary">Time (24-hour format)</h4>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const currentMinutes = selectedTime.includes(':') 
                        ? parseInt(selectedTime.split(':')[1]) 
                        : 0;
                      handleTimeSelect(hours, currentMinutes);
                    }}
                  >
                    {hours.map((hour) => (
                      <option 
                        key={hour} 
                        value={hour}
                        selected={selectedTime.includes(':') && parseInt(selectedTime.split(':')[0]) === hour}
                      >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const currentHours = selectedTime.includes(':') 
                        ? parseInt(selectedTime.split(':')[0]) 
                        : 0;
                      handleTimeSelect(currentHours, minutes);
                    }}
                  >
                    {minutes.map((minute) => (
                      <option 
                        key={minute} 
                        value={minute}
                        selected={selectedTime.includes(':') && parseInt(selectedTime.split(':')[1]) === minute}
                      >
            ...
            {/* أو اختيار المدة المحددة مسبقًا */}
            <div>
              <h4 className="text-sm font-medium mb-2 text-text-primary">Duration Presets</h4>
              <div className="grid grid-cols-3 gap-2">
